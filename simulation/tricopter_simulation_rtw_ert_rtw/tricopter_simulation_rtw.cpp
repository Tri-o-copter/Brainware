  ...
  real_T y[3])
{
  ...
  if (std::abs(A[3 + THREE]) > std::abs(A[3 + r_0])) {
    TWO = r_0 + 1;
    r_0 = THREE;
    THREE = TWO - 1;
  }
...
void tricopter_simulation_rtwModelClass::step()
{
  ...
  if (tricopter_simulation_rtw_DWork.DiscreteTimeIntegrator1_SYSTE_b != 0) {
    ...
    RxInCollective = 0.0025 * rtb_Sum2_e +
      tricopter_simulation_rtw_DWork.DiscreteTimeIntegrator1_DSTAT_m;
  }
  ...
  if (rtb_rad2deg_s_idx < 0.0) {
    rtb_rad2deg_s_idx = -std::sqrt(-rtb_rad2deg_s_idx);
  } else {
    rtb_rad2deg_s_idx = std::sqrt(rtb_rad2deg_s_idx);
  }
  ...
  rtb_TrigonometricFunction1 = std::cos(rtb_delta2);

  /* Trigonometry: '<S24>/Trigonometric Function' */
  rtb_TrigonometricFunction_h = std::sin(rtb_delta2);

  /* Sqrt: '<S29>/Sqrt' incorporates:
   *  DiscreteIntegrator: '<S4>/Rates'
   *  DotProduct: '<S29>/Dot Product'
   */
  rtb_Sqrt = std::sqrt((tricopter_simulation_rtw_DWork.Rates_DSTATE[0] *
                        tricopter_simulation_rtw_DWork.Rates_DSTATE[0] +
                        tricopter_simulation_rtw_DWork.Rates_DSTATE[1] *
                        tricopter_simulation_rtw_DWork.Rates_DSTATE[1]) +
                       tricopter_simulation_rtw_DWork.Rates_DSTATE[2] *
  ...
  rtb_rad2deg_s_idx = (tricopter_simulation_rtw_DWork.Velocity_DSTATE[0] * 0.0 +
                       rtb_GetDCM_0[0]) * 0.0;
  rtb_rad2deg_s_idx_0 = ((rtb_Direction * std::sin(rtb_DLookupTable) +
    rtb_GetDCM_0[1]) + tricopter_simulation_rtw_DWork.Velocity_DSTATE[1] * 0.0) *
    0.0;
  rtb_rad2deg_s_idx_1 = ((tricopter_simulation_rtw_DWork.Velocity_DSTATE[2] *
    ...
    rtb_Direction_o) + rtb_Direction * std::cos(rtb_DLookupTable)) +
    rtb_GetDCM_0[2])) + tricopter_simulation_rtw_DWork.Velocity_DSTATE[2] * 0.0)
    * 0.0;
  ...
  rtb_GetDCM_0[1] = (rtb_Direction_o - rtb_Direction_d) *
    tricopter_simulation_rtw_ConstB.Gain_a + rtb_Direction * std::sin
    (rtb_DLookupTable) * tricopter_simulation_rtw_ConstB.Sqrt_n;
  rtb_GetDCM_0[2] = rtb_Direction * std::sin(rtb_DLookupTable) * 0.5;
  tmp[0] = 0.0;
  tmp[1] = 0.0;
  ...
  tricopter_simulation_rtw_DWork.DiscreteTimeIntegrator_DSTATE_e = 0.0025 *
  ...
  tricopter_simulation_rtw_DWork.DiscreteTimeIntegrator1_DSTAT_n = 0.0025 *
    rtb_Sum2_i + RxInRoll;
  ...
  tricopter_simulation_rtw_DWork.DiscreteTimeIntegrator_DSTATE_a = 0.0025 *
    RxInRoll + imax;
  ...
  tricopter_simulation_rtw_DWork.Velocity_DSTATE[0] = ((rtb_Init_idx_1 -
    rtb_Product2_a_idx) + rtb_rad2deg_s_idx) * 0.0 * 0.005 +
    tricopter_simulation_rtw_DWork.Velocity_DSTATE[0];
  tricopter_simulation_rtw_DWork.Velocity_DSTATE[1] = ((rtb_Init_idx_2 -
    rtb_Product2_a_idx_0) + rtb_rad2deg_s_idx_0) * 0.0 * 0.005 +
    tricopter_simulation_rtw_DWork.Velocity_DSTATE[1];
  tricopter_simulation_rtw_DWork.Velocity_DSTATE[2] = ((rtb_Product1_h_idx -
    rtb_Product2_a_idx_1) + rtb_rad2deg_s_idx_1) * 0.0 * 0.005 +
    tricopter_simulation_rtw_DWork.Velocity_DSTATE[2];

  /* Update for DiscreteIntegrator: '<S4>/Position' */
  tricopter_simulation_rtw_DWork.Position_DSTATE[0] = 0.005 * rtb_Product_e_idx
    + tricopter_simulation_rtw_DWork.Position_DSTATE[0];
  tricopter_simulation_rtw_DWork.Position_DSTATE[1] = 0.005 * rtb_Init_idx +
    tricopter_simulation_rtw_DWork.Position_DSTATE[1];
  tricopter_simulation_rtw_DWork.Position_DSTATE[2] = 0.005 * rtb_Init_idx_0 +
    tricopter_simulation_rtw_DWork.Position_DSTATE[2];
}
